...
function setPositionBSS(bss_id, valign) {
...
function initBSS(bss_id, width, height, align, valign, titleSize, titleTop) {
	Cufon.replace('#' + bss_id + ' h2', {fontFamily:'Myriad Pro Regular'});
	$('#' + bss_id + ' h2:first').css({
	...
	$('#' + bss_id + ' .gui-bss-iframe-banner').css({
		width:width + 'px',
		height:height + 'px'
	});
	$('#' + bss_id + ' .gui-bss-template-01-top a.close:first').click(function() {
		var mainBlock = $(this).parents('.gui-bss-template-01');
		$.jajax('/jajaxserver.php?app=BannerSliderSystem&action=close', {
			post:'id=' + mainBlock.attr('id'),
			fun:$.jajaxparse
		});
		mainBlock.slideUp('slow');
		return false;
	});
	if ($.browser.msie && $.browser.version == 7) {
		setPositionBSS(bss_id, valign);
		$(window).scroll(function() {
			setPositionBSS(bss_id, valign)
		});
	}
	$('#' + bss_id).css(css).slideDown('slow');
}